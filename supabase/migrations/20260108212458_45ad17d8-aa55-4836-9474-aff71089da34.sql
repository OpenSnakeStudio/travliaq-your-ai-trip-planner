-- 1. Add public read policy for countries table (reference data)
CREATE POLICY "Anyone can read countries"
ON public.countries
FOR SELECT
USING (true);

-- 2. Add admin check to insert_trip_from_json function
...
BEGIN
  ...
  IF NOT public.has_role(auth.uid(), 'admin') THEN
    RAISE EXCEPTION 'Only admins can insert trips';
  END IF;
  ...
  -- Insertion des steps avec support multilingue, step_type et summary_stats
